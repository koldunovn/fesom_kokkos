...
## CORE2 864 CPU: the screen PASSES (job 26904576)

Base and the raced winner KaMinPar `w=100+nlev`, 3,000 steps at dt 1800, one allocation, both
legs rc=0 with matching global diagnostics at step 3,000 (`uv` 1.62/1.61, identical T/S ranges):
base 0.0447 s/step, KaMinPar 0.0425 s/step — **−4.92 %** in-allocation, corroborating the raced
−4.07 % and, like every other point re-measured at length (Finding 32), coming back slightly
larger. Stability column: ✅. The accuracy gate runs as job 26904852 (three seed controls at
`dist_864` — 424242 · 777001 · 990013 — generated by partgens 26904849–51).

### CORE2 864 accuracy gate, first pass (job 26904852): BORDERLINE — the 3-control envelope cannot judge it

Base + KaMinPar + three seed controls (424242/777001/990013), 20 steps at dt 1800, all legs rc=0,
halo gate ok, SSH iterations in class (mean |Δ| 0.150 vs controls up to 0.100, max |Δ| = 1 like
two of the three controls). Fields vs base:

| var (rms) | KaMinPar | control envelope (3) | verdict |
|---|--:|--:|---|
| temp | 5.994e-02 | 6.372e-02 … 6.726e-02 | **below every control** |
| salt | 2.239e-01 | 4.586e-02 … 1.805e-01 | 24 % above the top |
| ssh | 7.047e-03 | 3.545e-03 … 6.514e-03 | 8 % above the top |

But every displayed salt quantile — p50, p99, p99.99, max — is *inside or below* the control
envelope; only the rms is out, and the controls themselves span **×3.9** on salt rms, the same
under-spanning fArc showed (×2.7 across four controls, Finding 38 context). Per the standing rule
(§2.3 of the s4→s5 handoff): a too-narrow envelope condemns innocent arms, and widening is *not*
