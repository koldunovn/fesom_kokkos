...
| oce_fluxes | `fesom_ice_oce_fluxes` — **M4.3d-b** (host until then) | ice `flx_h/flx_fw`, tracers S, forcing Ssurf, sr scalars | forcing `heat_flux`,`water_flux`,`virtual_salt`,`relax_salt` | maps + 2 `integrate_nod_2D` reductions (parallel_reduce + Allreduce, the CG-dot shape) + 4 halos; `→host(forcing)` before the ocean step reads them |
