...
device FCT(S) → device Redi(S) → [host tracer-diff 13b + sfloor, M2.7] → bolus-sub `_kk`. (a) **M2.6-b
...
scatters + the bolus map added no new divergence class on the pi mesh). **The whole ocean step is now
device-resident through substep 13 except the M2.7 `impl_vert_diff_tracers` + sfloor and the §5 CG.**
