...
| EVP | ✅ **`fesom_ice_evp_dynamics_kk` (whichEVP=0) (M4.3b — DONE)** | ice `a/m/ms`,`srfoce_u/v/ssh`,`stress_atmice_*`,`uice/vice`,`sigma*` + set-once mesh | ice `uice/vice`, work `sigma*/eps*/inv_*/rhs_*/ice_strength` | **the 120-subcycle island = the CG/M4.2 pattern: HOST loop (fixed 120, no reduce) + DEVICE per-subcycle kernels (`stress_tensor_kk` per-elem / `stress2rhs_kk` map+SCATTER+map / save-old / velocity-update) + setup Steps 1-4 (4 kernels). 2 element→node SCATTERS (D22, atomic_add): Step 3 elevation-grad rhs + stress2rhs Loop 2 — Serial bit-id, OpenMP/CUDA climate-close (compounds over 120 subcycles). Driver IN `modify_host()+sync_device()` on a/m/ms/srfoce/stress_atmice/uice/vice/sigma (L28); OUT `sync_host(uice/vice/sigma)`. The COASTAL BC stays the verbatim C edge loop on the HOST (needs `partit->myList_edge2D`), folded into the per-subcycle uice/vice halo bracket's host phase (free at np>1; device-mask BC = M5). Verify `evp` = L26 capture-before uice/vice/sigma11/12/22 (the rheology RMW state). CORE2-only (L42/L43).** |
| FCT | ✅ **`fesom_ice_tg_rhs_kk` + `fesom_ice_fct_solve_kk` (M4.3c — DONE)** | ice `uice/vice`, `data[*].values`, the set-once `fct_massmatrix` + `ssh_stiff` CSR | ice `data[*].values` (RMW), work `values_rhs/valuesl/dvalues/fct_*` | **3 toolkit patterns composed (the M2.6 ocean-FCT analogue, 2-D): `tg_rhs` = zero-map + element→node SCATTER into `values_rhs` (no halo, consumers read OWNED); `solve_low/high_order` = the M4.2 CG per-row CSR-gather SpMV over the `ssh_stiff` sparsity + `fct_massmatrix` (race-free → bit-id Serial+OpenMP) — `high_order` is the CG/EVP host-loop (iterate device sweeps + per-iter `dvalues` halo); 3× Zalesak `fem_fct` = antidiff-flux map + cluster-min/max gather + `icepplus/icepminus` SCATTER + correction/limit maps + apply (overwrite `vals=valuesl` + `vals += fluxes` SCATTER). 3 element→node SCATTERS (D22, atomic_add: tg_rhs / fem_fct +/- sum / fem_fct vals apply) → Serial bit-id, OpenMP/CUDA climate-close. `fct_massmatrix` shares the ssh_stiff sparsity → one-shot push in `mass_matrix_fill` (rowptr/colind already device-current from the preconditioner). Driver IN `modify_host()+sync_device()` on `uice/vice` + the 3 `values` (L28; FCT-internal scratch fully recomputed → not pushed); OUT `sync_host(data[*].values)` for the host cut_off+thermo. 21 internal D21 halo brackets (valuesl ×3 / dvalues ×9 / icepplus+icepminus+values per tracer ×9), `npes>1`-guarded. Verify `icefct` = L26 capture-before the 3 values (CORE2-only, L42/L44).** |
...
| thermo | ✅ **`fesom_ice_thermodynamics_kk` (M4.3d-a — DONE)** | ice a/m/ms,uice/vice,srfoce,thermo.t_skin/thdgr; forcing runoff/Ch/Ce; the 8 JRA55 physics arrays (now Field-backed) | ice `data[*].values`(+`_old`), `thermo.t_skin/thdgr/thdgrsn/thdgra/thdgr_old/ustar`, `flx_h/flx_fw` | **per-node column physics over [0,N) — race-free map → bit-identical Serial AND OpenMP, NO scatter (the M2.7 TDMA shape). `therm_ice`/`obudget`/`budget`/`flooding`/`tfrez` = `KOKKOS_INLINE_FUNCTION` device twins taking a POD `IceThermC` (the fesom_ice_thermo scalars; its Field members can't cross to device). Loop 1 (ustar) owns its nod2D halo (Loop 2 reads ustar at halo). The 8 jra arrays Field-wrapped (M4.3d-a data layer, the M1.4 pattern — jra was the only non-Field input; CORE2-only). Driver IN `modify_host()+sync_device()` on all inputs (jra const→const_cast, L28); OUT `sync_host` the 9 consumed outputs. Verify `icethermo` = L26 capture-before the 5 RMW inputs (m/ms/a/t_skin/thdgr). CORE2-only (L42/L45).** |
| oce_fluxes | ✅ **`fesom_ice_oce_fluxes_kk` (M4.3d-b — DONE)** | ice `flx_h/flx_fw` (device-current from thermo, no push), tracers S, forcing Ssurf, sr scalars | forcing `heat_flux`,`water_flux`,`virtual_salt`,`relax_salt` | **maps over [0,N) + 2 `integrate_nod_2D` GLOBAL reductions (`parallel_reduce` + `MPI_Allreduce` — the M4.2 cg_dot shape: Serial seq → bit-identical, OpenMP/CUDA climate-close, D22) + owned net-subtract maps (⚠️ virtual_salt subtract skips cavity, relax_salt does NOT — match the C). The kernel owns its 4 forcing halos: `→host(forcing)` then host exchange; the ocean step's tracer-diff IN rail re-pushes the halo'd host forcing → device (the device-island handoff). flx_h/flx_fw flow device→device from the M4.3d-a thermo (no push). Driver IN pushes tracers S (const→const_cast) + forcing Ssurf. EOS-style verify (full overwrite from intact inputs → no capture-before), key `iceflux`. CORE2-only (L42/L46).** |
