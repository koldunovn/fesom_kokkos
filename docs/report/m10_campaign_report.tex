...
37.7\,\% (fArc on 8 GPU nodes). Below roughly 5\,\% the effect is within measurement noise;
...
& fArc, 8 nodes & 19949 & 37.7 & \texttt{oati} & $-7.32$\,\% \\
...
CPU and GPU coverage is complete for CORE2, fArc and DARS. NG5 has no valid CPU row: at
$\geq$4096 ranks the model reaches its own vertical-CFL blow-up guard between steps 150 and
175. That limit is documented for this mesh, timestep and cold start, and Fortran reaches it
at step 203 against the port's 200, so measuring NG5 requires either a window shorter than
about 120 steps or a smaller timestep. NG5 on GPU rests on a single 16-node configuration.
