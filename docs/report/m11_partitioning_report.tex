...
\date{\normalsize 15 August 2026 --- Levante (128-core CPU nodes, 4$\times$A100 GPU nodes; GH200 testbed)}
...
\item Renumbering the mesh nodes along a Hilbert curve pays only on CORE2 ($-5.4\,\%$
...
The shipped partitions cost the GPU backend up to 18\,\% of the model step. Most of that is
...
\midrule
DARS  & 64 GPU   & $\mathbf{-18.5\,\%}$ & \minconn+\contig+\texttt{UF=30} & clean & within; smallest T change of any leg \\
DARS  & 64 GPU   & $\mathbf{-12.9\,\%}$ & \minconn            & clean & within (T, ssh); S at the edge \\
NG5   & 64 GPU   & $\mathbf{-10.1\,\%}$ & \texttt{UFACTOR=30} & clean & within, all three fields \\
NG5   & 64 GPU   & $\mathbf{-9.5\,\%}$  & \minconn            & clean & within, all three fields \\
...
at the time step given in Section~1, all measured under the deterministic
initial-condition fill. The last column compares each candidate's twenty-step rms difference
from the reference with the range that different METIS seeds of the shipped recipe produce;
every candidate here falls inside that range, and Section~6 gives the numbers. On NG5 at
2{,}048 CPU processes no candidate was faster than the shipped partition.}
\label{tab:board}
\end{table}
...
GPU node, and combines almost additively with repartitioning ($-5.4\,\%$ at 512 processes).
...
\midrule
withdrawn, ``largest change measured'' & DARS 64 GPU, \minconn+\contig+UF30 & within; smallest T change of any run \\
not recommended, on accuracy & fArc 16 GPU, \minconn+\contig & within; smallest T change of any run \\
salinity rms $+24\,\%$ & CORE2 864 CPU, KaMinPar & at or below all five seed runs \\
ssh $+11\,\%$, solver cause & DARS 64 GPU, \minconn & within; identical iteration path \\
ssh $+8\,\%$, cause unknown & NG5 64 GPU, \minconn & within, all three fields \\
never completed a run & NG5 64 GPU, \texttt{UFACTOR=30} & within, all three fields \\
\bottomrule
\end{tabular}
\end{center}
...
For runs on the machines measured here: try \minconn\ first on GPU --- it is the best or
near-best candidate at four of the five GPU points --- and decide \contig\ and \texttt{UFACTOR}
by a timing run, since they help on some meshes and cost on others. NG5 at 64 processes is the
exception that shows why the timing run is not optional: there the plain \texttt{UFACTOR=30}
candidate is the fastest of all, half a point ahead of \minconn. On CPU
use KaMinPar or Mt-KaHyPar with $w = 100 + \mathrm{nlev}$, or \texttt{UFACTOR=30} if adding a
partitioning program to the workflow is not worth it: at CORE2 512 the two are equal, at fArc
the program is 2.5 percentage points better, and at DARS 2{,}048 the program is the
\emph{slowest} of the options --- there, regenerating the shipped partition with a new random
seed ($-4.6\,\%$) is as good as anything and costs one partitioner run. That is specific to
DARS; on CORE2 864 and fArc a new seed is 3--15\,\% slower than the shipped partition, so test
it rather than assume it either way. Hilbert renumbering is worth taking on
...
One question is open. Whether an interconnect with working GPU-direct communication prices
messages like the A100 fabric or like the GH200 test system decides how much of the GPU gain
transfers to JUPITER, and only a measurement there will settle it.

The question that dominated the first version of this report --- whether the candidates with
large temperature or salinity excursions were safe to use --- has been answered by removing
the excursions: they were the decomposition-dependent initial condition of Section~5. Every
recommended candidate now lies inside the seed scatter, so none of them carries a specific
reason for a long paired integration beyond the general one that applies to any change of
partition. All measurements in this report were made under \texttt{FESOM\_IC\_EXTRAP=det}.
