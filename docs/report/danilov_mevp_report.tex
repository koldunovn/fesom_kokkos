...
\begin{tabular}{@{}lrrr@{}}
\toprule
CORE2, one node, four GPUs, all six in one allocation & model step & \multicolumn{2}{c}{sea-ice cost} \\
\midrule
every sub-cycle (reference)                  & 0.0667\,s          & 15.7\,ms         & --- \\
\emph{delayed exchange (stale halo), $K$=8}  & \emph{$-14.4\%$}   & \emph{6.6\,ms}   & \emph{$-58.0\%$} \\
wide halo, standard form, separate kernels   & $-3.0\%$           & 13.9\,ms         & $-11.5\%$ \\
wide halo, divergence form, separate kernels & $+1.1\%$           & 16.1\,ms         & $+2.6\%$ \\
\textbf{wide halo, standard form, widened loops}   & $\mathbf{-14.1\%}$ & \textbf{6.8\,ms} & $\mathbf{-56.7\%}$ \\
\textbf{wide halo, divergence form, widened loops} & $\mathbf{-14.4\%}$ & \textbf{6.5\,ms} & $\mathbf{-58.6\%}$ \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{The trade between speed and a clean ice field does not exist.} The delayed exchange is in
the table because Sect.~6.4 rejects it for putting the decomposition into the ice field while
recording that it saves about three times what the wide halo saves. It no longer does: in the
divergence form the widened-loop wide halo matches it exactly on the model step and is 1.5\% ahead
of it on the ice cost, and in the standard form it is within a third of a percent. The wide halo
still computes the ring and still refreshes it exactly every window, so it still leaves no
imprint. The factor of three was our kernel structure, and with it goes the only argument this
study had for a stale halo.

\textbf{The two forms become indistinguishable.} They differ by 3.5 percentage points with
separate ring kernels and by nothing to four decimals with widened loops, although the divergence
form still ships 40\% fewer numbers. Sect.~6.5's decomposition of that gap into a message term and
a byte term is not withdrawn --- both terms were isolated at bit-identical output --- but what it
decomposed was smaller than the implementation term sitting on top of it.
...
for this variant rather than a diagnostic.
...
merely re-read stored stresses, that costs 5 percentage points at 512 cores on fArc. In the
divergence form, where the gather had to recompute each element's stress once per incident ring
node, about six times over, it saves 3.
