...
not (section~6.2). Second, the cheap option, the one that gives up exactness, turns out to
\textbf{imprint the domain decomposition on the ice} (section~6.6), and that, rather than its
speed, is what decides the recommendation.
...
\subsection*{6.6 The delayed exchange imprints the domain decomposition}
...
\textbf{Why this is a different kind of defect from an accuracy cost.} Section~6.5 says the
delayed exchange stays inside mEVP's own implementation spread, and that remains true. But the
error is not distributed like an implementation difference --- it is organised along artificial
lines, and \emph{it depends on how the mesh was partitioned}. The same experiment run on a
different number of cores has a different error pattern. That is a property a climate model
should not acquire lightly, and it is not visible in any of the integral diagnostics of
section~6.5.
...
free of an accuracy or reproducibility argument. Its $-19.1\%$ of the ice cost on fArc is the same
measurement as your $\sim$20\%. \\[0.3em]
delayed exchange &
approximate; $-37$ to $-58\%$ of the ice cost on GPU --- three times the wide halo --- but
\textbf{it imprints the domain decomposition} (edge/interior 2.52 at $K$=4 against a control of
0.85) &
\textbf{not recommended as it stands}, despite being much the fastest. Its error is inside mEVP's
own implementation spread but is organised along sub-domain boundaries and depends on the core
count. Worth revisiting only with evidence about a decade-scale integration, or with a hybrid that
refreshes exactly now and then. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}\itemsep0.12em
\item \textbf{Whether the partition imprint matters on climate time scales.} Two months on fArc
establish that it exists, is immediate and is visible; they say nothing about whether it biases
ice thickness or drift over decades, which is the question that would decide whether the delayed
exchange is usable at all. This is now the largest gap.
\item \textbf{The accuracy of the delayed exchange at high resolution.} Established at CORE2
resolution over one year; the speed results that motivate it are on fArc, DARS and NG5, where the
ice is better resolved and the velocity field is rougher.
