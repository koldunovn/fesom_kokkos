...
wide halo, standard form, separate kernels   & $-2.4\%$           & 14.1\,ms         & $-10.8\%$ \\
wide halo, divergence form, separate kernels & $+1.1\%$           & 16.3\,ms         & $+3.2\%$ \\
\textbf{wide halo, standard form, widened loops}   & $\mathbf{-13.6\%}$ & \textbf{6.7\,ms} & $\mathbf{-57.6\%}$ \\
\textbf{wide halo, divergence form, widened loops} & $\mathbf{-13.6\%}$ & \textbf{6.6\,ms} & $\mathbf{-58.2\%}$ \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{The saving was ours to give away, not the scheme's to withhold.} 58\% of the ice cost is
the upper end of the range we measure for the delayed exchange (Sect.~6.4), the scheme we reject
for putting the decomposition into the ice field. The statement in Sect.~6.3 that the wide halo is
worth substantially less than the delayed exchange on every mesh holds for the implementation
measured there, not for the scheme.
...
costs one conditional. It is much the fastest of the three: at the four operating points it takes
$-14.4$, $-14.3$, $-15.5$ and $-9.9\%$ off the model step at $K=8$ (CORE2, fArc, DARS, NG5) and
$-12.6$, $-11.4$, $-14.2$ and $-8.5\%$ at $K=4$, which against the ice cost is $-37$ to $-58\%$ on
GPU and $-5$ to $-25\%$ on CPU --- about three times what the wide halo gives. On CPU the same
change is worth $-1$ to $-3.4\%$ of the step, because a CPU process holds far less of the mesh.

We nevertheless do not regard it as usable. The stale values lie on the one-cell boundary of each
