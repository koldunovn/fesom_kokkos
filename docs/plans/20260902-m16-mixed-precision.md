...
| D13 | **Scaling is scoped to the SP paper and deferred (user, 2026-09-09).** The scaling campaign this track owes is CPU **and** GPU strong scaling *of single precision alone*, measured **without** the three M14 optimisations — ice (M9 mEVP wide halo), solver (M10 SSH solvers), partitioning (M11) — because those belong to the *next* paper, which sits on top of single precision. It is **not** run now: the session's purpose is the port plus its faithfulness against the Fortran mixed-precision oracle. Consequence: G2 needs one prize-sizing point per posture, not a ladder; the M14 recipe rows (D8) are next-paper data, not SP-paper data. **Open at the boundary, confirm before the campaign:** split-explicit SSH (`FESOM_SSH_MODE=se`, M12/M12b) and the upstream-default preconditioner (#984 `precond_variant=1`, now shipped by upstream) were NOT named among the three — the preconditioner is upstream's default so it is baseline; split-explicit is a scheme, not one of the three levers, so it is read as baseline-eligible but unconfirmed. |
| D14 | **The faithfulness matrix (2026-09-09): what G4 actually compares, and against what bar.** G4's stated bar — "SP-vs-Fortran ≡ DP-vs-Fortran to the printed digit" — had no runnable form until the oracle existed. Its concrete shape is now four arms on the ONE setup both codes can share, upstream's `setups/test_core2` (CORE2 mesh + JRA55 + PHC): **Fortran-DP, Fortran-SP, port-DP, port-SP**, plus **FP64 noise twins** built with upstream's own `&oce_perturb` (gaussian σ=2e-4 K applied at the first step, Suvarchal's seeds 12345/22318). The verdict is a **ratio, not a distance**: relL2(SP,DP) computed inside each code, and required to agree between the codes — with the noise twins setting the floor, because a departure smaller than the model's own response to a rounding-sized nudge is not evidence about precision. Driver `scripts/m16_faith_setup.sh` + `jobs/job_m16_faith_{fortran,port}`, analysis `scripts/m16_faith_compare.py`. **Four deviations from upstream's setup.yml, each forced by "both codes must run the same physics", each argued in the script banner:** (1) the mesh is our private CORE2 copy — /pool is *not* the same mesh, its `nlvls.out`/`elvls.out` were rewritten 2026-07-03 and differ at 2 nodes / 4 elements (made shallower), and bathymetry is not roundoff; (2) `use_ocean_only_forcing=.false.`, the Fortran *source* default, because the port has no `sftof` mask at all; (3) output precision 8, not upstream's 4 — a single-vs-double study must not push its own diagnostic through a float write; (4) upstream's `fcheck:` truth values are **not** the anchor (their tool ships only in the CI docker image, and they were made on /pool's mesh at prec 4) — our own Fortran-DP arm is the reference, and the published values are at best a cross-check on it. **`step_per_day`:** upstream's CI value 32 (dt 2700) is measurably marginal on a cold start — the 1-month pilot logged CFLz_max 2.46 against the 1.75 warning threshold — so production runs use 48 (dt 1800, this project's CORE2 protocol). Asking a precision question at a configuration's stability limit would report the timestep as if it were the precision. |
| D15 | **Split-explicit SSH is NOT baseline; the SP paper runs everything with CG (user, 2026-09-10).** This closes the boundary question D13 left open. The paper-1 scaling campaign therefore uses the plain CG solver — no `FESOM_SSH_MODE=se`, no `FESOM_SE_M` — on every mesh and both precisions, alongside the already-settled exclusions (M9 ice, M10 solvers, M11 partitioning). The #984 preconditioner remains baseline because upstream ships it as the default. Consequence for the board: the fArc and dars **recipe** rows in §1, which carry `FESOM_SSH_MODE=se`, are paper-2 data — the paper-1 CPU numbers for those meshes are the **knobs-off** rows (fArc 4096 0.855, dars 8192 0.732). |
...
- [ ] **in progress 2026-09-07** — CORE2 1N GPU (4 A100): DP 0.0618 / SP 0.0531 s/step, **SP/DP 0.859**, CG 60/62, job 27289143 (`docs/MIXED_PRECISION_M16.md` §1); CORE2 16N GPU job 27289163 + NG5 16N job 27289174 queued; **fArc 4096 CPU SP/DP 0.855 (job 27289249), dars 8192 CPU 0.732 (job 27289250)** — user lifted the compute-node cap 2026-09-07. Spec: pairs, 300 steps, `WSPLIT=1` where the mesh needs it: CORE2 4N + 16N GPU · fArc CPU at 4096 ·
