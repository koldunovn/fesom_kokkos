...
- [x] **CP2 / ACCEPTANCE — ✅ DONE.** 1-yr CORE2 CUDA climate **PASS** (`m32_cuda_m515_1yr`, 17275 steps, 0.1424 s/step on CORE2 = −17.5% vs M5.14's 0.1727): apples-to-apples vs M5.14 1-yr differ only in the 4th–5th sig-fig (D22 floor) — sst/ssh corr 1.00000, sss/a_ice/m_ice 0.99996–0.99998, uice 0.99978 (vs-Fortran 0.85019 = the known C↔F budget). **Zero climate-fidelity cost — M5.15 COMPLETE.** L60 written. T4 (uvnode) deferred (marginal).
  - **🎯 NEXT-LEVER MEASUREMENT (forcing-phase split, gated FPROF in `fesom_main.cpp:1045`): `force:bulk_compute` = 0.5517 s/step = 16.0% of the step** (single-threaded HOST bulk math — the blmc/L49 trap, nsys-invisible), `force:jra55_read` = 0.098 s (2.8%, stays host). **⟹ porting `fesom_bulk_compute` to a device per-surface-node map BEATS Lever-B overlap: ~16% contained win, climate-safe (race-free map → bit-identical Serial), AND it dissolves the T/uvnode residency wall (their ~760 MB/step DtoH disappears once bulk reads them on-device). This is the recommended next campaign (M5.16), not overlap-B.**
