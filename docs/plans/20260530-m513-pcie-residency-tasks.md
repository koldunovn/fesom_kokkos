...
- [~] **M5.13g1-T** — tracer `T` values full residency — **ATTEMPTED → FAILED the CUDA gate → REVERTED** (not committed). All 13 edits applied (T re-pushes removed, both halos→`fesom_halo_field`, ocean2ice T push removed, init+pre-I/O added); Serial rungs PASSED (verify=0, pi bit-id, SYNCCHECK) but the **CORE2-active-ice gate FAILED: T diverged 5.0e-2 (ceil 1e-2), propagating to Kv 5.4e-1 / S / u / v / h_ice** — a real CUDA staleness (Kv-worst ⇒ T stale at the EOS ⇒ wrong density ⇒ everything). **Likely cause: the `values`/`valuesold` device-coherence asymmetry** — I made `values` device-resident but left `valuesold` host-staged, and the MFCT/AB2 (`valuesAB`) couples them. The gate CAUGHT it (pi/Serial couldn't — host==device). Attempt saved: `/work/ab0995/a270088/port2/m513_ckpt/g1T_attempt.diff`.
  - **Follow-up:** flip `valuesold` TOGETHER with `values` (full residency for both + a valuesold init push) so the MFCT reads a coherent pair on device, OR leave T host-staged. Then re-gate. (S stays host-staged — floor-pinned — regardless.)
...
- [x] **Acceptance** — NG5 dist_16 clean timing. **a–f: step 16.27→10.88 s/step, GPU/CPU 3.76×→2.51×. +g1-uv: step →6.97 s/step (−57% total), GPU/CPU →1.61× — BELOW the ~2× target** (vs CPU dist_512=4.330, unchanged — flips are `#ifdef CUDA`). PCIe (nsys a–f) 12.74→7.48 s/step (−41%); g1-uv removed ~3.9 s/step more.
