...
- [ ] **Acceptance (CLIMATE FIDELITY)** — 1-yr CORE2 CUDA (KPP, 1958) on the campaign binary vs Fortran-KPP + C-port-KPP (`scripts/m32_climate_compare.py`) — **IN FLIGHT (~2.5–3h)**. PASS = corr~1, CUDA-vs-C ≤ C-vs-Fortran (the pre-campaign O(1e-4) scatter-drift signature, `docs/REFERENCE_RUNS.md`). The per-milestone 20-step gate is a *staleness* tripwire, NOT this climate test.
