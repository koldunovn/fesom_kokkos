...
- [x] **M5.13g1-uv** — `uv` (ELEM3D) **full device-residency** — DONE ✅ flip update_vel halo + REMOVE all 11 uv re-pushes + the ocean2ice cross-file push (`fesom_ice.cpp`) + step-1 init push + pre-I/O; verify pp/vrhs/vfilt/ivisc/ssh/ale/tradv=0 (260 lines); bit-id; SYNCCHECK; gate PASS 4.7e-3; **deep_copy 163.8→150.9/step (−13), 641.4→342.3 MB/step (−299, biggest MB drop — uv is the largest field). PROVED the cross-file full-residency approach works** (g1-uv spans into ocean2ice and passed clean).
- [x] **M5.13g1-T** — tracer `T` values full residency — **DONE ✅ (the gate caught 2 wrong attempts first).** T values + valuesold both device-resident (coherent pair); removed ALL T re-pushes (EOS/GM/FCT/Redi/trdiff) + the ocean2ice T push; T+valuesold init push + T→pre-I/O. **The real cause of the 2 earlier gate FAILs (deterministic T 5.0e-2) was NOT valuesold (ruled out — flipping it changed nothing) — it was `fesom_bulk_compute` reading `SST = T[surface]` on the HOST every step** (`fesom_bulk.cpp:259`, the JRA55 air-sea heat flux — exactly the **L50 uvnode→bulk** pattern). FIX = one targeted T `sync_host` after trdiff (T device-resident *with* a bulk SST sync, like uvnode). verify eos/kpp/gm/tradv/trdiff=0; pi np1+np2 bit-id; SYNCCHECK; **gate PASS worst 1.4e-2 (T 5e-2→1.6e-3)**; deep_copy 150.9→139.9/step (−11), 342.3→277.3 MB/step.
...
- [x] **Acceptance (PERFORMANCE)** — NG5 dist_16 clean: step **16.27→10.88 (a–f)→6.97 (+g1-uv)→6.12 s/step (+g1-T) = −62%**; node-for-node GPU/CPU **3.76×→1.41×** (vs CPU dist_512=4.330, unchanged — flips are `#ifdef CUDA`). PCIe (nsys a–f) 12.74→7.48 (−41%); g1-uv ~−3.9, g1-T ~−0.85 s/step.
- [x] **Acceptance (CLIMATE FIDELITY)** — **✅ PASS** (run `25236304`, `m32_cuda_m513_1yr`, rc=0, 17280 steps). 1-yr CORE2 CUDA (KPP, 1958) on the campaign binary vs Fortran-KPP + C-port-KPP (`scripts/m32_climate_compare.py`): backend-vs-C **statistically identical to the pre-campaign M5.9-pin run** on every field (sst corr 1.00000/bias 1.2e-4; sss/ssh/a_ice/m_ice/uice match to 3–4 sig figs) → **zero climate-level change from the flips**; pre vs post differ only in the 4th–5th sig fig (run-to-run GPU non-determinism). vs Fortran all corr ≥0.99996 except uice 0.85 (pre-existing genuine C↔F ice-velocity budget, GPU-independent). **Retires the "1.623e-3?" worry** — the 20-step gate's worst cell does NOT accumulate. `docs/GPU_FIDELITY.md` § Climate validation.
- [x] **Docs** — L57 (corrected, `7013c0f`) + scaling (`docs/SCALING_NG5.md` §M5.13 + dars cross-mesh) + fidelity (`docs/GPU_FIDELITY.md` §M5.13 + climate PASS) + figures (`scaling_meshsize_trend.png` campaign overlay, `2 m513 figs polished`) updated. Plan kept in `docs/plans/` (NOT moved to `completed/` — ~5 memory/doc references point to this path; marked complete in place).
