...
- [~] **M5.13g1-T** — tracer `T`/`S` values full residency — still DEFERRED (more complex than uv: the `valuesold` channel + EOS/GM/ocean2ice T pushes + S is floor-pinned so only T flips clean). g1-uv de-risked the method → g1-T is now a lower-risk follow-up. See § Deferred.
