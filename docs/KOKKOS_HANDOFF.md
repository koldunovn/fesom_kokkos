...
**Bottom line: (1) launch fusion caps at <1%/lever (L55); (2) the mesh-size lever ASYMPTOTES — CORE2 8.9× → farc 5.5× → dars 4.1× → NG5 3.8× (node-for-node GPU/CPU), increments collapsing (−3.4,−1.4,−0.3) → ~3.6-3.8× is the Levante-A100 floor for this port, reached by dars-class meshes. Bigger meshes won't close it. (Trend ratios are Kokkos-CUDA/Kokkos-Serial; the 8.9×/5.5× were vs the faster C-port — see `docs/figures/scaling_*.png` which use the consistent Kokkos/Kokkos 5.3×→3.8×.) (3) THE REGIME SHIFTS WITH MESH SIZE (L56): NG5 profile (dist_16) is COMPUTE/bandwidth-bound (FCT 22.8%, GM 11%, ALE 9%, KPP 8%, trdiff/velrhs 7%, ivisc 6% ≈ 72%; FCT scales LINEARLY 41×/43×-work), NOT launch/halo-bound like CORE2 (where SSH+ice_dyn+CG ≈ 28% collapse to <8% at NG5). → the M5.12 fusion levers are dev-regime; the PRODUCTION lever is Lever C (rank-1→View<double**> coalescing + OMEGA vertical-scratch TeamPolicy) which "failed" at CORE2 (1.3%) only because CORE2 wasn't bandwidth-bound. M5.13 = scoped Lever-C retry on FCT/GM validated at dars/NG5; next datum = ncu bound labels on the NG5 top kernels.**
