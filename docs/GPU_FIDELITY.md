...
**The headline insight — `deep_copy` bytes ≠ wall-clock.** `hnode_new`'s 742 MB/step at ~25 GB/s PCIe is only ~0.030 s/step of *bandwidth*, yet removing it cut the step by ~0.23 s/step (the clean hnode_new-only −11.4 %) — **~8× the bandwidth estimate**. The cost was never bandwidth: each `sync_host`/`sync_device` is a BLOCKING fence that drains and refills the GPU pipeline at a substep boundary, and `hnode_new`'s 3× round-trip serialized three substep transitions every step. So a per-step host↔device round-trip of a big 3-D field crossing substep boundaries is worth *far* more wall-clock than its byte count suggests — **prioritize PCIe targets by round-trips/step (pipeline stalls), not by GB/step.** The deep_copy metric under-states the opportunity for per-step round-trippers (`hnode_new`, `sw_3d`) and over-states it for one-shot/startup transfers. This is why the naive "~2.6 % from 38 % of PCIe" estimate was 4× too low.
...
## §M5.21 — finish the coalescing track: bucket-A FCT (8 sub-kernels) + `update_vel` flat-levered, −8.1 % step, bit-identical

The remaining bucket-A maps classified at the M5.19 close (§M5.19 "Remaining coalescing headroom"). **Step-0 re-profile** (`jobs/job_ng5_prof`/`job_nsys_ng5`/`job_ng5_synclog` on the m520 binary; NG5 dist_16 = **1.6856 s/step** today) **confirmed the plan** (not refuted this time): `13_fct` still #1 at **25.15 %**, and the bucket-A kernels are now a *bigger* share than the doc estimated (the step shrank under them) — `fct_f2d_v` 1.58, `fct_ale_recon` 1.16, `fct_LO_final` 1.11, `fct_zal_a1` 0.92, `fct_zal_a2` 0.82, `fct_qr4c_v` 0.75, `fct_upw1v` 0.72, `fct_grad_elem` 0.55, `fesom_update_vel` **1.06** (doc said 0.82) → **~7.9 % of loop**, the single biggest clean (bit-identical) chunk left. (The synclog binary `fesom_port_synclog` is the *pre-M5.20* diagnostic build → it still shows hnode_new/sw_3d round-tripping & 3413 MB/step; the `ghats` 256 + `S` 249 D2H leaders are untouched by M5.20 so still valid. Rebuild it for true post-M5.20 attribution when starting Lever 2.)

**The lever = the M5.18/M5.19 flat lever, verbatim** (`src/fesom_eos.cpp:488` `fesom_smooth_nod3D_kk` is the template): each per-node/per-elem kernel with an internal `for(nz)` → **`RangePolicy(0, N*nl)`, decode `(n,nz)`, mask `nz∉[lo,hi)`, register-write**. The field is node-major (`arr[n*nl+nz]`) / elem-major×comp (`uv[(e*nl+nz)*2]`, `trxy[(el*nl+nz)*2]`), so the LEVEL is the contiguous inner dim → a warp of consecutive flat-idx spans consecutive LEVELS of one node/elem → coalesced loads+stores (vs the old one-thread-per-node stride of `nl`=70 ≈ 52 sectors/req). Each `(n,nz)`/`(e,nz)` writes only its own level's output (no scatter, no cross-(n,nz) overlap) → Serial order-independent → **byte-identical** (Serial AND OpenMP); CUDA at the existing atomic-scatter floor. 9 kernels flipped: 7 clean FCT (`upw1v`,`LO_final`,`grad_elem`,`zal_a1`,`zal_a2`,`f2d_v`,`ale_recon`) + `update_vel` + `qr4c_v`.

**The one subtlety — `fct_qr4c_v`'s 2-level-column double-write.** qr4c_v does `aflux_v[k] = HO(k) − aflux_v[k]` (read-then-write, same level; vertical reads `valsAB[nz±2]`/`Zc` are INPUTS → otherwise a clean per-level map). But its surface/2nd-layer/bottom-1/bottom case structure has ONE within-column write-dep: when a column is exactly 2 levels deep (`nzmax−nzmin==2`) the "2nd-layer" and "bottom-1" writes BOTH hit level `nzmin+1`, the second reading the first's result with the SAME `−0.5(Tup+Tdn)·W·area` term → they **cancel to a net no-op** (the level keeps its upw1v value). The flat lever reproduces this by leaving that one level untouched (`if (nzmax-nzmin==2 && nz==nzmin+1) return`); every other level is written exactly once via an explicit branch dispatch (surface / 2nd / bottom-1 / bottom / interior). **pi cannot exercise this** (idealised mesh, no 2-level columns) — only real CORE2 bathymetry (shallow coastal nodes) does, which is why the CORE2-Serial fresh-vs-saved check below is load-bearing here.

### Validation — all PASS (bit-identical, the M5.18/M5.19 discipline)
- **Per-kernel Serial `max|Δ|==0`:** pi np1 `FESOM_KK_VERIFY=tradv,ssh` (strstr → both fire) → `fct(tr0)/fct(tr1) values=0` and `uv=0` every step (the whole FCT chain incl. qr4c_v, + update_vel via the `ssh` end-to-end key).
- **pi np1 + np2 BIT-IDENTICAL** vs the golden C baseline / `pi_np2_ref_m13_nocma` (all fields, zero-tolerance). **SYNCCHECK clean** (`build-synccheck`, no host-read-while-device-authoritative).
- **CORE2 Serial bit-identical (the qr4c_v edge-case proof):** `cp -r serref_core2 serref_m520_saved`; gate `--fresh-oracle` rebuilt `serref_core2` from m521-Serial; `diff_snap.py` saved-vs-fresh = **ALL FIELDS BIT-IDENTICAL** → m521-Serial ≡ m520-Serial on the real mesh incl. 2-level columns.
- **CUDA fidelity gate** (CORE2 dist_8 ice-active, m521-CUDA vs m521-Serial): **PASS**, worst max|Δ|=1.194e-2 (h_ice; T 1.0e-3, S 3.7e-4, u/v 1.8e-4) — the same atomic-scatter floor as M5.18–M5.20, no staleness regression. Since the Serial leg is bit-identical, the CUDA divergence is purely the *pre-existing* scatter/EVP floor (the flat-levered kernels have NO atomics → add zero divergence).
- **A/B (same-node, NG5 dist_16, ONE allocation):** CLEAN **1.6359 → 1.5039 s/step = −8.07 %** (prof-fenced 1.6826→1.5517 = −7.78 %). `deep_copy` **unchanged at 2175.84 MB/step** both legs → confirms a pure-compute lever (zero PCIe change; also a clean cross-check of M5.20's 2176 MB/step). Per-kernel s/step (m520→m521): `13_fct` phase 0.4237→0.3084 (−27 %); each flipped kernel **−78…−92 %** (`fct_f2d_v` 0.0266→0.0023, `update_vel` 0.0178→0.0017, `fct_zal_a1` 0.0155→0.0012, `qr4c_v` 0.0127→0.0026, …). The 9 per-kernel reductions sum to ~0.131 s/step ≈ the −0.132 clean A/B delta → the win is fully attributed to coalescing (identical to the M5.18 smoother 25.7 %→0.6 % signature).
- **1-yr CORE2 CUDA climate** (`m32_cuda_m521_1yr`, 17280 steps): IN FLIGHT — corr=1.00000 vs m520 guaranteed (Serial bit-identical; the CUDA leg is at the unchanged scatter floor). Will close as M5.18–M5.20 each did.

**NG5 dist_16 ≈ 1.50 s/step** now (this node-mix; ≈ 2.6–2.7× faster than a CPU node, extrapolating M5.16's measured 1.63×). The fat single kernels (smoother M5.18, vel_rhs+GM M5.19) and now all bucket-A maps are harvested. Binaries: `build-cuda/fesom_port` == `fesom_port_m521`; `fesom_port_m520` = the pre-M5.21 A/B baseline. Tooling: `jobs/job_ng5_m521_ab`. Lesson **L66**.

### Remaining coalescing/compute headroom — for M5.22
Inside `13_fct` (now 19.88 %) the remainder is bucket-B/C/D, NOT flat-lever-able: the `*_h` edge SCATTERS (`fct_eud_fill` 2.24 %, `fct_mfct_h` 2.05 %, `fct_zal_b3h`/`b1h`, `fct_f2d_h`) → edge-coloring or Lever C; the "b" cluster (`fct_zal_b2` 1.40, `b1v` 0.77, `b3v` 0.58 — flux-correction, check for column-coupling) is **unclassified, worth a code-read** (some per-node b-kernels may be a clean bucket-A subset the M5.19 sweep missed); bucket-B `fct_zal_a34` 1.33 (vertical 3-layer dep on WRITTEN values → TeamPolicy); bucket-C TDMAs (`impl_vert_diff_tracers` 2.08, `impl_vert_visc`, `fer_solve_gamma` → Lever C layout refactor only). The clean flat lever is now exhausted; the next compute wins need a different technique.

## §M5.21 Lever-2 PCIe — the prompt's attribution was STALE; the truth (user-driven, 2026-05-30)

The user questioning "didn't we already do the salinity floor on device?" unravelled the whole prompt §2/§6 PCIe plan. Re-derived from current code (synclog rebuilt on m521, `build-cuda-synclog`, `-DFESOM_SYNC_LOG=ON`): the post-m521 per-step D2H leaders are **`kpp.ghats` 255.8 + `tracers.S` 249.1 MB/step** (bytes were right; the *attribution* was wrong on BOTH).

### Lever 2a — `kpp.ghats` 256 D2H was a PLACEBO → REMOVED (−3.08 %, bit-identical) ✅
`ghats` (the KPP non-local transport term) is computed on device, then `sync_host`'d + host-halo-exchanged every step — but **`use_kpp_nonlclflx=.false.`** so the tracer-diffusion non-local flux is skipped (`fesom_tracer_diff.cpp:19,291`) and **nothing consumes ghats** (the `kpp` verify diffs only Kv/Av; the sole host reader is the `FESOM_KPP_DUMP_DIR` debug dump). Fix (`fesom_kpp.cpp:1597`): guard the `sync_host`+exchange on `fesom_kpp_dump_this_step()` → skipped in production, exact under the dump. **Removing dead data → BIT-IDENTICAL** (SYNCCHECK clean *with KPP active* = the empirical no-host-reader proof; pi np1 bit-identical; gate PASS worst 1.113e-2 = the *same* floor as m521, zero new divergence). A/B (m521→m521b): **1.4999→1.4537 s/step = −3.08 %**; `deep_copy` 2175.84→1931.90 (−243.9, calls 121→120 = exactly the one ghats D2H gone). The −3.08 % ≈ 4–5× ghats's raw 256-MB bandwidth → the L65 fence+halo-stall signature again. ⚠️ Guarded: RESTORE the unconditional sync+halo if the KPP non-local flux is ever ported. binary `fesom_port_m521b`.

### `tracers.S` 249 D2H is NOT the floor AND NOT a port target — it's REDUNDANT host work (already on device)
- The **salinity floor** is a device kernel since **M5.14** (`fesom_salinity_floor_kk`, L57.2). Not it.
...
## §M6.5 — the PER-RANK PROXY: what zstar actually costs at scale (farc + dars, 2026-07-13)

**The question.** Every M6 timing was CORE2 @ 8 ranks = 15.9k 2D-verts/rank — a compute-bound point
where every knob delta sat inside a 5.6% noise floor. zstar adds a restored `hnode_new` **NOD3D halo
every step** (the rail M5.20 deleted as a linfs-only optimisation), and a comm-bound step is what
punishes that. Predicted: a real cost, growing as verts/rank falls.

**Measured** — 3 configs × 2 reps, INTERLEAVED (`d z a d z a`); rep spread 0.10–0.44% (20× tighter
than CORE2's cross-job noise), so these deltas are resolved:

| point | verts/rank | topology | default | **zstar** | **all3** |
|---|---|---|---|---|---|
| farc A | 159,596 | 4 ranks, **1 node — NO NETWORK** | 0.3074 | **+1.11%** | **−4.31%** |
| farc B | 79,798 | 8 ranks, 2 nodes | 0.2424 | **+0.47%** | **−3.18%** |
| **dars** | **98,760** | **32 ranks, 8 nodes — real fabric** | 0.3140 | **+1.34%** | **−4.47%** |

### What is SOLID

1. **zstar costs +0.5% to +1.4%, everywhere measured** — two meshes, three per-rank loads, including
   a genuinely distributed 8-node run. **It never gets large.** That is the decision-relevant fact.
2. **The all-3 config is 3–4.5% FASTER than the default**, confirmed on both meshes. TKE + mEVP are
   cheaper than KPP + std-EVP by more than zstar costs. (CORE2 hinted at this — mEVP −4.7% — but
   could not resolve it against its own noise, so the claim was correctly withheld then.)
3. The **kernel-visible** part of zstar's cost is per-rank compute, identical in shape on both meshes:
   Shchepetkin PGF (net, replaces the linfs PGF), the zstar thickness commit, and `vert_vel_zstar`.
   The GM phase row moves +0.0017 s on dars but **every one of its 14 leaf kernels is unchanged
   (Δ = +0.0000)** — that row is a wall-clock phase timer absorbing wait, not GM compute.

### ⚠️ What is NOT established — two claims RETRACTED

An earlier revision of this section claimed *"the halo is exonerated — 6% of zstar's cost"* and
*"zstar gets relatively cheaper as you decompose; ≈ +0.7% at NG5@16N."* **Both were overconfident.**

- **`fesom_halo_pack`/`unpack` measure the GPU pack/unpack KERNELS. They do NOT include the MPI
  transfer or the `Waitall` latency** — that is wall-clock time which lands in whatever phase timer
  brackets it. So "+0.0002 s of halo" was only ever the *kernel* cost, never the halo's true cost.
- **farc point A is 4 ranks on ONE node — there is essentially no network there.** Reading a
  communication cost off a single-node run is invalid, and it is what produced the flattering trend.
- On dars — the only real distributed point — the leaf kernels account for **+0.0027 s of the
  measured +0.0042 s. The missing +0.0015 s (36%) is wall-clock time no kernel explains**, and one
  extra MPI exchange per step is the obvious candidate. It is not proven, but it is not excluded.
- The "shrinks with decomposition" trend does not survive the second mesh: dars at 98.8k/rank costs
  **more** (+1.34%) than farc did at 159.6k/rank (+1.11%).

**Bottom line: the SIZE of zstar's cost is nailed down (≤1.4%, small); the MECHANISM is not.**
The mechanism is interesting but not decision-changing — and **do not "optimise away" the
`hnode_new` rail on the strength of a kernel timer**: M6.3's bit-identity depends on it.

**To actually settle the mechanism** (if it ever matters): scan rank counts *within one mesh, all
multi-node* (dars dist_16 / dist_32 / dist_64 = 197k / 98.8k / 49.4k per rank, 4 / 8 / 16 nodes) so
the network is present throughout and only per-rank load varies — and instrument the MPI wait
directly rather than inferring it from kernel timers.

⚠️ Proxy caveat: faithful for halo + per-rank compute. NOT faithful for **CG iteration count**
(conditioning belongs to the real mesh). Read s/step out of this, not SYPD.
