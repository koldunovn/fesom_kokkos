...
| **SSH CG arithmetic**: the sparse-row accumulators of `A·v` and `M⁻¹·v`, every dot (`s_old`, `s_aux`, `sprod`), `rtol`, α, β, and the X/rr/pp recurrences — `dbl_t` accumulate, `real_t` store, `MPI_DOUBLE` reduces | `fesom_ssh.cpp` `cg_spmv`/`cg_dot`, `fesom_ssh_solve_cg_kk` baseline body, host twin `fesom_ssh_solve_cg` | **2** | `solver.F90` **@ a62f180** :150 `real(kind=WP_full) :: sprod(2), s_old, s_aux, al, be, rtol`; :194-196, :207-208, :240-241, :304-313 cast every sparse-row term to `WP_full` and round once into the WP vector. Upstream's own comment: *"they set the search direction and the stopping test — so rounding here steers the iteration itself rather than just reporting on it. The vectors (rr/zz/pp/App) and the matrix stay WP: the bandwidth is theirs, the accuracy is these."* | 🔴 **the registry's old class-3 row asserted "upstream: all WP" — that is false for the oracle.** The port flipped the whole chain to `real_t` in Phase B3 on that reading; §3r reverts it | **fixed 2026-09-11 (§3r)**, no knob. Gate 0 **PASS np1 AND np2, byte-identical** (dbl_t == real_t in FP64 ⇒ every cast is the identity). ⚠️ **measured as a NO-OP for the SP faithfulness gap** (1 month: day-31 salt ratio 1.937 vs 1.948) — kept because it is what upstream does, not because it bought anything |

| **Where the explicit tracer tendencies accumulate** — Redi/GM horizontal and explicit-vertical diffusion into `del_ttf`, ONE deferred ALE reconstruction | `fesom_gm.cpp` (4 write sites + 2 host twins), `fesom_tracer_adv.cpp` (`fesom_tracer_ale_recon_kk`), `fesom_step.cpp` | **2** | `oce_ale_tracer.F90`: `diff_part_hor_redi` :1744-1748 and `diff_ver_part_redi_expl` :1572 accumulate `del_ttf`; the reconstruction at :775-778 divides by `hnode_new` ONCE | 🔴 not a *type* island — a **placement** one, and the one that actually mattered. The port composed the two into `values += rhs*dt/(areasvol*hnode_new)`, rounding `S + increment` at float ulp(35) = **2.4e-06 psu** once per tracer per step instead of rounding an increment at ulp ~1e-12. `fesom_tracers.h` already carried the rule — *"Do NOT apply diffusion directly to values"* | **fixed 2026-09-11 (§3v)**, no knob. **Closes the SP faithfulness gap: month-1 salt ratio 1.95 → 1.02.** FP64 is NOT neutral ⇒ **Gate 0 re-based** (pi np1/np2 + CORE2 np8), with the reverted-source provenance proof |
...
|---|---|---|---|---|
| ~~CG scalar chain~~ — **🔴 THIS ROW WAS WRONG; SEE §1 (2026-09-11)** | | | | **REVERTED 2026-09-11, §3r** |
...
- **2026-09-08 — the multi-node CUDA intermittent NaN is NOT first-leg: it is the FIRST IN-LOOP FORCING REFRESH.**
  ...
  **Lead (2026-09-08, G4 screens):** two more deaths — CORE2 16N SP leg at step 2001 (job 27294510) and NG5 16N SP
  ...
  (jobs 27294800, 27294801) vs `=100000` (27294802): **print every step 0/10 failed, never-in-loop 1/5 (step 2 again)** — the
  print does not trigger it; the per-step host syncs may even suppress it (0/10 at a ~7 % base rate is suggestive, not
