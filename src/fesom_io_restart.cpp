/*
 * fesom_io_restart.cpp — see fesom_io_restart.h and
 ...
 *
 * ------------------------------------------------------------------
 * What this port adds over the C reference: the sync discipline
 * ------------------------------------------------------------------
 * Every array here is a fesom::Field, so the raw host pointer the gather reads
 * is only one of two copies. The restart is a host-side operation on state the
 * device may own, so each entry in the table carries its Field as well as its
 * host pointer, and:
 *
 *   write — Field::sync_host() before the gather, or a CUDA run checkpoints
 *           whatever the host mirror last happened to hold. On Serial the two
 *           views alias, so this is a no-op and the Serial gate cannot catch a
 *           missing one. That is why the table pairs base with fld rather than
 *           leaving the syncs to the call site: a field added without its
 *           Field is a compile-time hole, not a CUDA-only silent wrong answer.
 *   read  — Field::modify_host() then sync_device() after the scatter and the
 *           halo exchange, so the device sees the restored state before the
 *           first kernel reads it.
 *
 * const_cast on write follows the precedent already set in fesom_ssh.cpp
 * (FESOM_SSH_DUMP): sync_host mutates the DualView's bookkeeping and its host
 * mirror, never the model values.
 *
 * One deliberate difference from the C tree, which splits gather/scatter into
 * fesom_io_gather.c so the restart and its unit test need not link the whole
 * output subsystem: here every test links the single fesom_core object library
 * anyway, so the split would buy nothing and the collectives stay in
...
typedef struct {
    ...
    real_t         *base;      /* local host array, NULL = not present   */
    fesom::Field   *fld;       /* the DualView behind base — see banner  */
                     ...
                     struct fesom_tke        *tke)
{
    ...
    /* The write path takes mesh const; the table needs writable pointers on the
     * read path and Field handles on both. One alias, so the cast is stated
     * once instead of at every entry. */
    struct fesom_mesh *meshw = (struct fesom_mesh *)mesh;
    int n = 0;

    /* --- ocean, node, surface ---------------------------------------- */
    v[n++] = rst_var{ {"ssh", NULL}, dyn->eta_n, &dyn->eta_n_fld, 0, 1, 1, FESOM_HALO_NOD2D };
    v[n++] = rst_var{ {"ssh_rhs_old", NULL}, dyn->ssh_rhs_old, &dyn->ssh_rhs_old_fld, 0, 1, 1, FESOM_HALO_NOD2D };
    ...
    v[n++] = rst_var{ {"d_eta", NULL}, dyn->d_eta, &dyn->d_eta_fld, 0, 1, 1, FESOM_HALO_NOD2D };
    v[n++] = rst_var{ {"hbar", NULL}, meshw->hbar, &meshw->hbar_fld, 0, 1, 1, FESOM_HALO_NOD2D };
    v[n++] = rst_var{ {"hbar_old", NULL}, meshw->hbar_old, &meshw->hbar_old_fld, 0, 1, 1, FESOM_HALO_NOD2D };

    /* --- ocean, node, column ----------------------------------------- */
    v[n++] = rst_var{ {"hnode", NULL}, meshw->hnode, &meshw->hnode_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"zbar_3d_n", NULL}, meshw->zbar_3d_n, &meshw->zbar_3d_n_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"Z_3d_n", NULL}, meshw->Z_3d_n, &meshw->Z_3d_n_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"temp", NULL}, tr->data[FESOM_TRACER_T].values, &tr->data[FESOM_TRACER_T].values_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"temp_AB", NULL}, tr->data[FESOM_TRACER_T].valuesAB, &tr->data[FESOM_TRACER_T].valuesAB_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"temp_M1", NULL}, tr->data[FESOM_TRACER_T].valuesold, &tr->data[FESOM_TRACER_T].valuesold_fld,0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"salt", NULL}, tr->data[FESOM_TRACER_S].values, &tr->data[FESOM_TRACER_S].values_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"salt_AB", NULL}, tr->data[FESOM_TRACER_S].valuesAB, &tr->data[FESOM_TRACER_S].valuesAB_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"salt_M1", NULL}, tr->data[FESOM_TRACER_S].valuesold, &tr->data[FESOM_TRACER_S].valuesold_fld,0, nl, 1, FESOM_HALO_NOD3D };
    ...
    v[n++] = rst_var{ {"uvnode_u", "uvnode_v"}, dyn->uvnode, &dyn->uvnode_fld, 0, nl, 2, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"w", NULL}, dyn->w, &dyn->w_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"w_expl", NULL}, dyn->w_e, &dyn->w_e_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    v[n++] = rst_var{ {"w_impl", NULL}, dyn->w_i, &dyn->w_i_fld, 0, nl, 1, FESOM_HALO_NOD3D };
    ...
    v[n++] = rst_var{ {"tke", NULL}, tke ? tke->tke : NULL, tke ? &tke->tke_fld : NULL, 0, nl, 1, -1 };

    /* --- ocean, element ---------------------------------------------- */
    v[n++] = rst_var{ {"helem", NULL}, meshw->helem, &meshw->helem_fld, 1, nl, 1, FESOM_HALO_ELEM3D };
    v[n++] = rst_var{ {"dhe", NULL}, meshw->dhe, &meshw->dhe_fld, 1, 1, 1, -1 };
    v[n++] = rst_var{ {"u", "v"}, dyn->uv, &dyn->uv_fld, 1, nl, 2, FESOM_HALO_ELEM3D };
    v[n++] = rst_var{ {"urhs_AB", "vrhs_AB"}, dyn->uv_rhsAB, &dyn->uv_rhsAB_fld, 1, nl, 2, FESOM_HALO_ELEM3D };

    /* --- sea ice ------------------------------------------------------ */
    if (ice) {
        v[n++] = rst_var{ {"area", NULL}, ice->data[FESOM_ICE_AICE].values, &ice->data[FESOM_ICE_AICE].values_fld, 0, 1, 1, FESOM_HALO_NOD2D };
        v[n++] = rst_var{ {"hice", NULL}, ice->data[FESOM_ICE_MICE].values, &ice->data[FESOM_ICE_MICE].values_fld, 0, 1, 1, FESOM_HALO_NOD2D };
        v[n++] = rst_var{ {"hsnow", NULL}, ice->data[FESOM_ICE_MSNOW].values, &ice->data[FESOM_ICE_MSNOW].values_fld, 0, 1, 1, FESOM_HALO_NOD2D };
        v[n++] = rst_var{ {"uice", NULL}, ice->uice, &ice->uice_fld, 0, 1, 1, FESOM_HALO_NOD2D };
        v[n++] = rst_var{ {"vice", NULL}, ice->vice, &ice->vice_fld, 0, 1, 1, FESOM_HALO_NOD2D };
        /* Not in the Fortran restart: read before written in the
         * thermodynamics (fesom_ice_thermo.c:470 before :508). */
        v[n++] = rst_var{ {"t_skin", NULL}, ice->thermo.t_skin, &ice->thermo.t_skin_fld, 0, 1, 1, FESOM_HALO_NOD2D };
        /* Not in the Fortran restart: the EVP stress tensor is updated
         * incrementally and never zeroed on entry. */
        v[n++] = rst_var{ {"sigma11", NULL}, ice->work.sigma11, &ice->work.sigma11_fld, 1, 1, 1, -1 };
        v[n++] = rst_var{ {"sigma12", NULL}, ice->work.sigma12, &ice->work.sigma12_fld, 1, 1, 1, -1 };
        v[n++] = rst_var{ {"sigma22", NULL}, ice->work.sigma22, &ice->work.sigma22_fld, 1, 1, 1, -1 };
    }
                              ...
                              int **glen, int **gcol, T **gval)
{
    ...
    if (gp->mype == 0) {
        ...
        len_ro    = (int *)malloc((size_t)gp->total_n * sizeof(int));
                         ...
                         struct fesom_partit        *partit)
{
     ...
     *
     * fesom_update_stiff_mat_ale_kk is a DEVICE kernel that ends in
     * modify_device(), so under zstar the authoritative matrix lives on the
     * device from step 1 onward and the host mirror still holds the base
     * matrix. Pulling it is not optional here: without it a CUDA checkpoint
     * would carry the cold-start matrix and the resumed run would lose every
     * accumulated increment — the same failure the C gate caught, arriving by
     * a different route. const_cast follows the FESOM_SSH_DUMP precedent in
     * fesom_ssh.cpp: sync_host touches the mirror and the bookkeeping, never
     * the values. */
    ...
    for (int i = 0; i < nvars; ++i) {
        ...
        if (partit->mype == 0) {
            global = (real_t *)malloc(count * (size_t)stride * sizeof(real_t));
            plane  = (real_t *)malloc(count * (size_t)vars[i].levels * sizeof(real_t));
            FESOM_CHECK(global && plane, "restart: gather buffer alloc for %s",
                        vars[i].name[0]);
        }
        /* The host mirror is not the field: on CUDA the device may own it and
         * the host copy may be arbitrarily stale. Pull before gathering, or the
         * checkpoint records whatever was last synced for some other reason.
         * No-op on Serial, which is exactly why the Serial gate cannot catch a
         * missing one — see the banner. */
        if (vars[i].fld) vars[i].fld->sync_host();
        ...
        if (vars[i].is_elem && canon) {
            ...
            /* The canonicalisation changed the live array on the host, so the
             * device has to be told — otherwise the run continues from the
             * pre-canonicalisation device copy and the resumed run, which
             * starts from the file, diverges from it. */
                        ...
                        struct fesom_partit  *partit)
{
    ...
    *dt   = header_d[0];
    ...
    for (int i = 0; i < nvars; ++i) {
        ...
        /* Host is authoritative now; hand it to the device before any kernel
         * reads the field. Doing it here rather than at the call site keeps
         * the list of fields and the list of pushes the same list. */
        if (vars[i].fld) { vars[i].fld->modify_host(); vars[i].fld->sync_device(); }
    }
    ...
    /* The accumulated free-surface stiffness matrix. */
    {
        ...
        stiff->values_fld.modify_host();
        stiff->values_fld.sync_device();
    ...
    mesh->hnode_new_fld.modify_host();
    mesh->hnode_new_fld.sync_device();
