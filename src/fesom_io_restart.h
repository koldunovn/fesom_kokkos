/*
 * fesom_io_restart.h — restart write and read (Kokkos port).
 *
 ...
 *
 * Kokkos port: the format, the field list and the guards are the C reference's,
 * so the two write interchangeable files and Kokkos-Serial can be gated
 * byte-identical to C across a restart. What is added is the host/device sync
 * discipline — every field is a fesom::Field, pulled before the gather and
 * pushed after the scatter. See the banner in fesom_io_restart.cpp; the
 * stiffness matrix is the case that makes it load-bearing rather than tidy,
 * because under zstar the device owns it from step 1.
 *
