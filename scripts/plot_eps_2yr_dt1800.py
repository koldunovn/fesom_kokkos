#!/usr/bin/env python3
"""2-year validation figures: C port dt=1800 (AB2 epsilon fix, eps_2yr_dt1800) vs
Fortran+PP dt=1800 (fortran_pp_2yr). Same dt + scheme. Spatial triptych maps +
24-month global time series. Adapted from plot_validation_vsPP.py.

Run: PYTHONPATH=/home/a/a270088/PYTHON \
  /work/ab0995/a270088/mambaforge/envs/nereus/bin/python plot_eps_2yr_dt1800.py
"""
...
C = pathlib.Path("/work/ab0995/a270088/port/eps_2yr_dt1800")
...
en = np.asarray(nc.Dataset(DIAG)["face_nodes"][:])
en = en.T if en.shape[0] == 3 else en
en = en - 1 if en.min() == 1 else en
...
def triptych(cdat, fdat, ttl, cmap, vmin, vmax, dvlim, unit, proj, fname, mz=True):
    ...
    for i,(t,d,cm,lo,hi) in enumerate([("C port dt1800",cdat,cmap,vmin,vmax),
            ("Fortran dt1800",fdat,cmap,vmin,vmax),("C − Fortran",cdat-fdat,"RdBu_r",-dvlim,dvlim)]):
...
triptych(cvar("a_ice",2), fvar("a_ice",2), "a_ice Mar 1959 (NH max)", "Blues",0,1,0.3,"frac","np","map_aice_mar.png",mz=False)
...
def icearea(fn, hemi):
    ...
    for m in range(12):
        ...
        out.append(np.sum(a[sel]*node_area[sel])/1e12)
    return out
cSST=[];fSST=[];cSSS=[];fSSS=[];cNH=[];fNH=[];cSH=[];fSH=[]
for YR in (1958,1959):
...
for a,(c,f,ttl,u) in zip(ax.flat,[(cSST,fSST,"global-mean SST","degC"),
    ...
    a.plot(t,c,'-s',ms=3,label="C port (dt1800, eps fix)",color="tab:red")
    a.set_title(ttl); a.set_ylabel(u); a.set_xlabel("month (1958-1959)"); a.grid(alpha=.3); a.legend(fontsize=8)
fig.suptitle("2-year validation: C port (dt=1800, AB2 eps fix) vs Fortran (dt=1800, PP)",fontsize=13)
