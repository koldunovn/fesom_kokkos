#!/usr/bin/env python3
"""GPU step composition: CORE2 dist_8 (16k nod2D/rank, launch/halo-bound) vs NG5 dist_16
(462k nod2D/rank, compute-bound). Per-phase % of the timed step (FESOM_STEP_PROFILE=1).
...
ax.set_title("GPU step composition shifts from halo/launch-bound (CORE2) to compute-bound (NG5)",
...
txt=(f"compute phases:  {comp_c2:.0f}%  →  {comp_ng:.0f}%   (grow)\n"
     f"halo/launch (SSH+EVP):  {halo_c2:.0f}%  →  {halo_ng:.0f}%   (collapse)\n"
     f"CG share:  5.7%  →  0.3%\n"
...
leg=[Patch(fc="grey",alpha=0.45,hatch="///",ec="grey",label="CORE2 dist_8  (16k nod2D/rank)"),
     ...
     Patch(fc=COMP,label="compute-bound phase"),
     Patch(fc=HALO,label="halo / launch-bound phase")]
